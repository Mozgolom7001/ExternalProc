...
Функция ПолучитьСуммуВнесеннойПредоплаты(ДанныеПечати)
	...
	|				ИЗ
	|					АналикаУчетаПоПартнерам КАК АналикаУчетаПоПартнерам) {(ЗаказКлиента В (&ЗаказКлиентаОтбор)) КАК Поле2}) КАК РасчетыСКлиентами
	...
	Запрос.УстановитьПараметр("ЗаказКлиентаОтбор", ДанныеПечати.Ссылка.Договор);
	...
	Если не Результат.Пустой() Тогда
		...
		Выборка.Следующий();
		Возврат Выборка.НашДолгКонечныйОстаток;

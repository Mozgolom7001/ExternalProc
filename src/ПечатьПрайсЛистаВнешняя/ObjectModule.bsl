...
Процедура ЗаполнитьПостроительОтчетаПоЦенамНоменклатуры() Экспорт
	...
	|	КОНЕЦ КАК Цена,
                       ...
                       |			КОНЕЦ
                       |	КОНЕЦ / ЗапросПрайс.ЕдиницаИзмерения.Коэффициент КАК ЦенаШт,
                       |	ЗапросПрайс.Номенклатура.БазоваяЕдиницаИзмерения КАК ЕдиницаИзмеренияШт
	...
	|      МИНИМУМ(НетХарактеристик), МИНИМУМ(ЦеноваяГруппа), МАКСИМУМ(ЦенаШт), МИНИМУМ(ЕдиницаИзмеренияШт)
	|//ИТОГИ
	|ПО
	|	ЗапросПрайс.ТипЦен,
	|	ЗапросПрайс.ЦеноваяГруппа,
		...
		Если ЭтоНижнийУровень Тогда
			...
			Пока ВыборкаТипЦен.Следующий() Цикл
				Если ЗначениеЗаполнено(ВыборкаТипЦен.Цена) Тогда
					...
					//ЕЕК++
					Номенклатура = ВыборкаТипЦен.Номенклатура;
					КЕдИзм = ВыборкаТипЦен.ЕдиницаИзмерения.Коэффициент;
					
					СекцияТипЦен.Параметры.ЦенаШт = ОбщегоНазначения.ФорматСумм(Ценообразование.ОкруглитьЦену(ВыборкаТипЦен.Цена / КЕдИзм, ВыборкаТипЦен.ПорядокОкругления, ВыборкаТипЦен.ОкруглятьВБольшуюСторону)) + СтрВалюта;
					//ЕЕК--
				Иначе
				...
				Если мЕстьЕдиница Тогда
					...
					//ЕЕК++
					СекцияТипЦен.Параметры.ЕдиницаШт = ВыборкаТипЦен.Номенклатура.БазоваяЕдиницаИзмерения;
					//ЕЕК--
				КонецЕсли;
